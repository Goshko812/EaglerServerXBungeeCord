...
stop_servers() {
    ...
    docker exec -it mc-server-1-12-2 bash -c 'tmux kill-session -t limbo 2>/dev/null || true'
...
start_servers() {
    ...
    print_message "To attach to a server, use: docker exec -it mc-server-1-12-2 tmux a -t [bungeecord|server|limbo]"
    print_message "To change the MOTD edit server_motd: in persistent-storage-folder/eagler-server-1-12-2/bungeecord/plugins/eaglerxbungeecord/listeners.yml "
}
...
backup_servers() {
    ...
    docker exec -it mc-server-1-12-2 bash -c "cd /data && zip -r ${backup_name} eagler-server-1-12-2"
...
check_container_running() {
    if [ "$(docker ps -q -f name=mc-server-1-12-2)" ]; then
        return 0
    else
        return 1
    fi
